...
# -------------------------------------------------------------------
# KROK 3: Logika Hierarchicznego Liczenia (Bez zmian)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# ✅ KROK 4: "Kuloodporny" Endpoint `/api/project/create` (HYBRYDOWY)
# -------------------------------------------------------------------
...
def create_project_hybrid():
    ...
    if data_json:
        # Ścieżka 1: AI wysłało poprawny JSON
        print("INFO: Otrzymano poprawny format JSON.")
        keywords_state = parse_brief_from_json(data_json)
    else:
        # Ścieżka 2: JSON się nie udał. Próbujemy jako surowy tekst.
...
# -------------------------------------------------------------------
# KROK 5: Pozostałe Endpointy (Bez zmian)
# -------------------------------------------------------------------

@app.route("/api/project/<project_id>/add_batch", methods=["POST"])
def add_batch_to_project(project_id):
    # ... (kod bez zmian) ...
    if not db: return jsonify({"error": "Baza danych Firestore nie jest połączona."}), 503
    try:
        doc_ref = db.collection('seo_projects').document(project_id)
        doc = doc_ref.get()
        if not doc.exists: return jsonify({"error": "Projekt o podanym ID nie istnieje."}), 404
        ...
        if not batch_text: return jsonify({"error": "Brak tekstu w body żądania."}), 400
...
def health():
    return jsonify({
        "status": "ok",
        "version": "v5.3-hybrid-parser", # Wracamy do stabilnej wersji
...
def perform_s1_analysis():
    ...
    if not combined_text:
        ...
        return jsonify({
            "identified_urls": top_urls, "processing_report": source_log,
