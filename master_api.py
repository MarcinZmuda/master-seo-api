...
# --- NOWY ENDPOINT: /api/update_keywords ---
@app.route("/api/update_keywords", methods=["POST"])
def update_keywords():
    """
    Aktualizuje keywords_state po każdym batchu tekstu.
    Oczekuje JSON:
    {
      "text": "pełny tekst batcha",
      "keywords_state": {
          "keywords_with_limits": "fraza: 10-20x\ninna fraza: 3-7x"
      }
    }
    """
    data = request.get_json()
    ...
    # 1️⃣ Parsowanie keywords_with_limits
    ...
    # 2️⃣ Zliczanie wystąpień słów kluczowych w tekście (case-insensitive)
    text_lower = text.lower()
    for kw in limits.keys():
        count = len(re.findall(rf"\b{re.escape(kw.lower())}\b", text_lower))
        limits[kw]["used"] = count

    # 3️⃣ Walidacja zakresów i tworzenie raportu
    usage_summary = {}
    validation_report = []
    updated_lines = []
    ...
    # 4️⃣ Aktualizacja keywords_state
