...
# ===========================================================
# ⚖️ GEMINI JUDGE (Naprawiona wersja modelu)
# ===========================================================
def evaluate_with_gemini(text, meta_trace):
    ...
    # 🔥 ZMIANA: Używamy 'gemini-pro' zamiast 'flash', aby uniknąć błędu 404
    try:
    ...
    banned_phrases_list = """
    1. WYPEŁNIACZE: "W dzisiejszych czasach", "W dobie...", "Od zarania dziejów", "W niniejszym artykule".
    2. ŁĄCZNIKI: "Warto zauważyć", "Należy wspomnieć", "Warto dodać", "Co więcej", "Ponadto".
    3. ZAKOŃCZENIA: "Podsumowując", "Reasumując", "W ostatecznym rozrachunku", "Biorąc wszystko pod uwagę".
    4. IDIOMY: "Gra warta świeczki", "Strzał w dziesiątkę", "Szyte na miarę", "Klucz do sukcesu".
    5. ASEKURANCTWO: "Wszystko zależy od...", "Nie ma jednoznacznej odpowiedzi".
    6. WZMOCNIENIA: "Nie ma wątpliwości", "Bez wątpienia", "Z całą pewnością".
    7. ZNAKI: "—" (Długi myślnik/Pauza - AI często go nadużywa. Wymuszaj przecinki).
    """
    ...
    try:
        ...
        # FAIL OPEN: W razie awarii API Google przepuszczamy tekst z ostrzeżeniem
        return {
            "pass": True, 
            "quality_score": 80, 
            "feedback_for_writer": f"Gemini API Error: {str(e)}. Batch auto-accepted."
        }


# ===========================================================
# 🔧 Helpery do liczenia (Hybrid Row-Level Logic)
# ===========================================================
...
# ===========================================================
# 🧠 GŁÓWNA FUNKCJA (QUALITY GATE + ZLICZANIE UUID)
# ===========================================================
def process_batch_in_firestore(project_id: str, batch_text: str, meta_trace: dict = None):
    ...
    # Odrzucamy TYLKO jeśli pass=False LUB wynik < 70 (chyba że był błąd API i przeszło awaryjnie)
    ...
    # Iteracja po UUID (bo każdy wiersz ma ID)
    for row_id, meta in keywords_state.items():
        # Nazwa frazy jest wewnątrz obiektu 'keyword'
    ...
    return {
        ...
        "keywords_report": [
            {
                "keyword": meta.get("keyword", "Unknown"), # Wyświetlamy nazwę dla GPT
            ...
            # Sortujemy po nazwie frazy dla czytelności (ignorując ID)
