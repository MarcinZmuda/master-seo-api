...
def evaluate_with_gemini(text, meta_trace):
    ...
    # Używamy bezpiecznego modelu. Jeśli 1.5-flash nie działa, zmień na 'gemini-pro'
    ...
    try:
        ...
        # Fail Open (przepuszczamy w razie awarii Google)
        return {"pass": True, "quality_score": 50, "feedback_for_writer": "Gemini Error - Check Logs"}

# ===========================================================
# 🧠 HYBRID COUNTING (Exact + Lemma)
# ===========================================================
...
def process_batch_in_firestore(project_id: str, batch_text: str, meta_trace: dict = None):
    ...
    if not gemini_verdict.get("pass", True) or gemini_verdict.get("quality_score", 100) < QUALITY_THRESHOLD:
        return {
            ...
            "quality_alert": True,
            "info": "Odrzucono przez Gemini."
        }

    # 2. ZLICZANIE (UUID Loop)
    data = doc.to_dict()
    ...
    for row_id, meta in keywords_state.items():
        # Nazwa frazy jest teraz wewnątrz obiektu
    ...
    return {
        ...
        "keywords_report": [
            ...
            # Sortujemy po nazwie frazy, nie po UUID
