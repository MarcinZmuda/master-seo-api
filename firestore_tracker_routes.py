"""
SEO Content Tracker Routes - v15.6 STABLE
Backend przejmuje myślenie. GPT tylko wykonuje.
"""
...
def count_robust(doc, keyword_meta):
    ...
    # Lemma lookup - fallback to processing if not stored
...
def validate_quality(text, keywords_state, current_doc):
    ...
    if len(sentences) > 3:
        ...
        burstiness = (math.sqrt(variance) / mean) * 10 if mean > 0 else 0
        if burstiness < 6.0:
            return {"valid": False, "error": f"❌ Monotonia zdań (Burstiness: {burstiness:.1f}). Wymagane > 6.0. Przeplataj bardzo krótkie zdania z długimi."}
    
    # Density Check (Stuffing Guard)
    word_count = len(text.split())
    if word_count > 0:
        for row_id, meta in keywords_state.items():
            count = count_robust(current_doc, meta)
            # Check local density in this batch just for warning/blocking extreme stuffing
...
"""
    ...
    for meta in keywords_state.values():
        if meta.get("type") == "BASIC":
            ...
            if remaining_uses > 0:
                ...
                # Ensure at least 1 if falling behind
                target = per_batch
    ...
    # Add top 6 priority keywords to avoid overloading context
    instruction += "\n".join(targets[:6])
    
    # Add Extended reminder (Supporting phrases)
...
def preview_batch(project_id):
    ...
    for rid, meta in keywords_state.items():
        ...
        # Preview calculates "new total" if this batch were approved
