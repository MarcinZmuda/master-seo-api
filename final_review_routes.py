...
# ============================================================================
# 3. FIRESTORE PROCESSOR (⭐ UPDATED WITH HARD BLOCK)
# ============================================================================
def process_batch_in_firestore(project_id, batch_text, meta_trace=None, forced=False):
    ...
    # ⭐ STEP 1: COUNT keywords in batch WITHOUT updating cumulative
    batch_counts = {}
    ...
    # ⭐ STEP 3: If ANY keyword exceeded → BLOCK entire batch
    if blocked_keywords and not forced:
        return {
            "error": "KEYWORDS_EXCEEDED_MAX",
            "status": "BLOCKED",
            "blocked_keywords": blocked_keywords,
            "message": f"❌ {len(blocked_keywords)} keyword(s) would exceed target_max. Use synonyms or force approve.",
            "status_code": 400
        }
    ...
    for rid, meta in keywords_state.items():
        keyword_targets.append({
            ...
            "remaining_max": meta.get("remaining_max", 0),  # ⭐ CRITICAL!
